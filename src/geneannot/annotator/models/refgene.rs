...
mod test {
    ...
    fn test_parse_coding_transcript() -> Result<(), GeneAnnotError> {
        let index = Index::from_file(
            &"testfiles/geneannot/genome/human/GRCh38.primary_assembly.genome.fa.fai",
        )?;
    ...
    fn test_load_refgene() -> Result<(), GeneAnnotError> {
        let index = Index::from_file(&"testfiles/geneannot/genome/human/hg38.fa.fai")?;
        let genome = fasta::load_fasta("GRCh38", &index);
        load_refgene(
            genome,
            MultiGzDecoder::new(
                &include_bytes!("../../../../testfiles/geneannot/gene/UCSC/refGene.txt.gz")[..],
            ),
        )?;
    ...
    fn test_load_gencode() -> Result<(), GeneAnnotError> {
        ...
        load_refgene(
            ...
            MultiGzDecoder::new(
                &include_bytes!(
                    "../../../../testfiles/geneannot/gene/UCSC/wgEncodeGencodeBasicV33.txt.gz"
                )[..],
            ),
        )?;
