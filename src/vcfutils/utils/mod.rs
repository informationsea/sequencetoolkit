...
mod test {
    ...
    fn test_load_mapping() -> Result<(), VCFUtilsError> {
        let mapping = load_mapping(auto_csv_reader_from_path(
            "./testfiles/vcfutils/contig-mapping.csv",
            false,
        )?)?;
        assert_eq!(
            mapping,
    ...
    fn test_open_vcf_from_path() -> Result<(), VCFUtilsError> {
        open_vcf_from_path(Some("./testfiles/vcfutils/simple1.vcf"))?;
        open_vcf_from_path(Some("./testfiles/vcfutils/1kGP-subset.vcf.gz"))?;
        Ok(())
    }
}
