...
use crate::vcfutils::error::{VCFUtilsError, VCFUtilsErrorKind};
use nom::{
...
impl AlleleCount {
    pub fn new(alt_count: usize, ploidy: usize) -> AlleleCount {
        ...
        let mut genotype_count = Vec::new();
        for _ in 0..number_of_genotype(alt_count, ploidy) {
            genotype_count.push(0);
        }
    ...
    pub fn clear(&mut self, new_alt_allele_count: usize, new_ploidy: usize) {
        ...
        for _ in 0..number_of_genotype(new_alt_allele_count, new_ploidy) {
            self.genotype_count.push(0);
        }
