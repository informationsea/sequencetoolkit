...
use crate::vcfutils::utils::tablewriter::{TableWriter, XlsxDataType, XlsxSheetWriter};
use nom::branch::alt;
...
mod test {
    use super::*;
    use crate::vcfutils::utils::tablewriter;
    ...
    fn test_vcf2table_csv_split_multi() -> Result<(), VCFUtilsError> {
        ...
        assert_eq!(
            &write_bytes[..],
            &include_bytes!("../../../testfiles/vcfutils/simple1-expected-multiallelic-split.csv")
                [..]
        );
        Ok(())
    }
    ...
    fn test_vcf2table_csv_split_multi_snpeff() -> Result<(), VCFUtilsError> {
        let vcf_data = include_bytes!("../../../testfiles/vcfutils/simple1-snpeff.vcf");
        let config = VCF2CSVConfig {
            split_multi_allelic: true,
        ...
        assert_eq!(
            &write_bytes[..],
            &include_bytes!(
                "../../../testfiles/vcfutils/simple1-expected-multiallelic-split-snpeff.csv"
            )[..]
        );
        Ok(())
    }
    ...
    fn test_vcf2table_csv_split_multi_snpeff_with_canonical() -> Result<(), VCFUtilsError> {
        let vcf_data = include_bytes!("../../../testfiles/vcfutils/1kGP-subset-snpeff.vcf");
        let config = VCF2CSVConfig {
            split_multi_allelic: true,
            decoded_genotype: true,
            canonical_list: Some(
    ...
    fn test_vcf2table_csv_no_split() -> Result<(), VCFUtilsError> {
        ...
        assert_eq!(
            &write_bytes[..],
            &include_bytes!("../../../testfiles/vcfutils/simple1-expected-no-split.csv")[..]
        );
        Ok(())
    }
